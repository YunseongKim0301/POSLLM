...
class UnifiedLLMClient:
    """
    Ollama LLM 클라이언트 (포트 로테이션 지원 + 자동 시작)

    개선사항:
    - 포트 로테이션으로 부하 분산
    - 스레드 안전 포트 선택
    - 토큰 추적
    - Ollama 서버 자동 시작 (Astrago/웹터미널 환경 최적화)
    """

    # 클래스 레벨에서 Ollama 프로세스 추적 (여러 인스턴스가 동일 서버 공유)
    _ollama_processes: Dict[int, subprocess.Popen] = {}
    ...
    def __init__(self, ollama_host: str = "127.0.0.1", ollama_ports: List[int] = None,
        ...
        self.auto_start = auto_start_ollama
        ...
        if self.auto_start:
            ...
            # 프로그램 종료 시 Ollama 서버 정리
            atexit.register(self._cleanup_ollama_servers)

    def _find_ollama_binary(self) -> Optional[str]:
        """
        Ollama 바이너리 찾기 (여러 경로 검색)
    ...
    def _start_ollama_server(self, port: int) -> bool:
        """
        특정 포트에서 Ollama 서버 시작
        ...
        if not ollama_bin:
            self.logger.error(f"Ollama 바이너리를 찾을 수 없어 포트 {port}에서 시작 불가")
            return False

        with UnifiedLLMClient._ollama_lock:
            # 이미 시작된 프로세스가 있는지 확인
            if port in UnifiedLLMClient._ollama_processes:
                proc = UnifiedLLMClient._ollama_processes[port]
                if proc.poll() is None:  # 아직 실행 중
                    self.logger.info(f"Ollama 서버가 이미 포트 {port}에서 실행 중")
                    return True
            ...
            try:
                ...
                self.logger.info(f"Ollama 서버 시작 중: {ollama_bin} serve (포트 {port})")
                ...
                UnifiedLLMClient._ollama_processes[port] = process

                # 서버가 실제로 시작될 때까지 대기 (최대 30초)
                for i in range(30):
                    time.sleep(1)
                    if self._check_ollama_server(port):
                        self.logger.info(f"✓ Ollama 서버가 포트 {port}에서 정상 시작됨 ({i+1}초 소요)")
                        return True

                self.logger.warning(f"Ollama 서버가 포트 {port}에서 30초 내에 시작되지 않음")
                return False

            except Exception as e:
                self.logger.error(f"Ollama 서버 시작 실패 (포트 {port}): {e}")
                return False

    def _ensure_ollama_running(self):
        """
        모든 포트에서 Ollama 서버가 실행 중인지 확인하고, 필요시 시작
        """
        ...
        for port in self.ports:
            if self._check_ollama_server(port):
                ...
                self.logger.warning(f"✗ Ollama 서버가 포트 {port}에서 실행되지 않음 → 자동 시작 시도")
    ...
    def _cleanup_ollama_servers(self):
        """
        모든 Ollama 서버 프로세스 정리 (프로그램 종료 시)
        """
        with UnifiedLLMClient._ollama_lock:
            if not UnifiedLLMClient._ollama_processes:
                return

            self.logger.info(f"Ollama 서버 프로세스 정리 중: {len(UnifiedLLMClient._ollama_processes)}개")

            for port, proc in UnifiedLLMClient._ollama_processes.items():
                try:
                    if proc.poll() is None:  # 아직 실행 중
                        self.logger.info(f"Ollama 서버 종료 중 (포트 {port}, PID {proc.pid})")
                        proc.terminate()
                        proc.wait(timeout=5)
                except Exception as e:
                    self.logger.warning(f"Ollama 서버 종료 실패 (포트 {port}): {e}")

            UnifiedLLMClient._ollama_processes.clear()
